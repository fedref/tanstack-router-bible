...
## 문서에서 "다루는 범위"는 줄이지 않는다

2번(Simplicity First)은 **구현 방식**에 관한 규칙이다 — 같은 일을 200줄로 할 수
있으면 50줄로 하라는 것. 이 저장소에서도 코드는 그대로 따른다.

한편 `README.md` 의 제1원칙은 **다루는 범위**에 관한 것이다.

> **"어려우니 빼자"는 금지.** 난이도를 이유로 기능을 생략하면, 독자는 그 기능이
> *없다고* 믿게 된다. 이해 못 할 위험보다 **존재를 모를 위험이 크다.**

둘은 다른 축이라 부딪히지 않는다. 오히려 2번의 "요청받지 않은 것은 만들지 마라"
(범위를 넘지 마라)와 위 원칙(범위 안을 빠뜨리지 마라)은 **같은 방향을 양쪽에서
받친다.** 요청받은 것을 정확히 — 넘치지도 모자라지도 않게.

실전에서 이 저장소가 조심할 쪽은 "모자람"이다. 한때 `@tanstack/react-router` 가
export 하는 100개 중 22개만 문서화된 상태였고 아무도 알아채지 못했다. 그래서
문서를 쓸 때는 잘 안 쓰는 API·저수준 유틸·deprecated 항목까지 적되, "언제 쓰나 /
대개 안 쓴다 / 무엇으로 대체됐다"를 함께 밝힌다. 문서의 **서술은 간결하게, 다루는
범위는 빠짐없이.**

문서 범위를 줄이는 판단은 사용자의 몫이다. 임의로 축소하지 않는다.
